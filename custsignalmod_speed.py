...
OSC_THRESHOLD = 4 # Must be less or equal to number of items in OSC_INDICATORS
...
#     print(f'Module Path: {os.path.dirname(os.path.realpath(__file__))}')
    ...
    for pair_name, analysis in multiple_anal.items():
        ...
        if DEBUG: print(f'Custsignalmod:{pair_without_exchange} Oscillators:{oscilator_check}/{len(OSC_INDICATORS)} Moving averages:{moving_average_check}/{len(MA_INDICATORS)}')
